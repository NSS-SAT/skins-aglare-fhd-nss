...
from Components.NOVComponents import isImageType, isPY2
import os

DBG = False
if DBG: from Components.NOVComponents import NOVDEBUG
...
class NOVBitrate(Converter, object):
    ...
    @cached
    def getText(self):
        if DBG: NOVDEBUG("[NOVBitrate:getText] vcur %s" % self.vcur)
        if self.vcur > 0:
    ...
    def doSuspend(self, suspended):
        if DBG: NOVDEBUG("[NOVBitrate:suspended] >>> self.isSuspended=%s, suspended=%s" % (self.isSuspended,suspended))
        if suspended == 0:
            self.isSuspended = False
            self.StartTimer.start(100, True)
        else:
    ...
    def start(self):
        if self.isRunning == False:
            if self.source.service:
                if DBG: NOVDEBUG("[NOVBitrate:start] initiate runTimer")
                self.isRunning = True
                self.runTimer.start(100, True)
            else:
                if DBG: NOVDEBUG("[NOVBitrate:start] wait 100ms for self.source.service")
                self.StartTimer.start(100, True)
        else:
            if DBG: NOVDEBUG("[NOVBitrate:start] runBitrate in progress, nothing to do")

    def runBitrate(self):
        if DBG: NOVDEBUG("[NOVBitrate:runBitrate] >>>")
        if isImageType('vti'):
            demux = 2
        else:
            adapter = 0
        ...
        try:
            ...
            if stream:
                if DBG: NOVDEBUG("[NOVBitrate:runBitrate] Collecting stream data...")
            ...
            if DBG: NOVDEBUG("[NOVBitrate:runBitrate] Exception collecting stream data: %s" % str(e))
        try:
            info = self.source.service.info()
            vpid = info.getInfo(iServiceInformation.sVideoPID)
            apid = info.getInfo(iServiceInformation.sAudioPID)
        except Exception as e:
            if DBG: NOVDEBUG("[NOVBitrate:runBitrate] Exception collecting service info: %s" % str(e))
        ...
        if vpid >= 0 and apid >= 0:
            ...
            if DBG: NOVDEBUG('[NOVBitrate:runBitrate] starting "%s"' % cmd)
            self.container.execute(cmd)
    
    def clearValues(self, *args): #invoked by appClosed & kill from suspend
        if DBG: NOVDEBUG("[NOVBitrate:clearValues] >>>")
        self.isRunning = False
    ...
    def appClosed(self, retval):
        if DBG: NOVDEBUG("[NOVBitrate:appClosed] >>> retval=%s, isSuspended=%s" % (retval, self.isSuspended))
