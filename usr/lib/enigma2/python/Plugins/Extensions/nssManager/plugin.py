...
FILE_XML = os.path.join(plugin_path, 'nssManager.xml')
FTP_XML = ''
FTP_CFG = 'http://nonsolosat.net/nssManager/cfg.txt'
# nonsolosat.net/nssmanager
local = True
...
class nssManager(Screen):
    ...
    def configtv(self):
        from Plugins.Extensions.nssManager.data.datas import nssConfig
    ...
    def action(self):
        ...
        if self['list'].getCurrent():
            ...
            cmd = 'chmod 755 /usr/camscript/*.*'  # + self.softcamslist[self.var][0] + '.sh'
            os.system(cmd)
            # chmod(cmd, 775)
            curCam = self.readCurrent()
            foldcurr = '/usr/bin/' + curCam
            chmod(foldcurr, 755)
        ...
        try:
            ...
            png1 = LoadPixmap(cached=True,
                              path=resolveFilename(SCOPE_PLUGINS,
                                                   "Extensions/nssManager/res/img/{}".format('actcam.png')))
            png2 = LoadPixmap(cached=True,
                              path=resolveFilename(SCOPE_PLUGINS,
                                                   "Extensions/nssManager/res/img/{}".format('defcam.png')))
            if s >= 1:
                for lines in scriptlist:
                    ...
                    for line in sfile:
                        if line[0:3] == 'OSD':
                            ...
                            print('We are in nssManager and cam is type  = ', nam)
                            if self.currCam != 'None' or self.currCam is not None:
    ...
    def autoclean(self):
        ...
        for line in myfile.readlines():
            print('We are in nssManager line, self.oldService.toString() =', line, self.oldService.toString())
...
class nssGetipk(Screen):
    def __init__(self, session):
        ...
        FTP_XML = 'http://nonsolosat.net/nssManager/nssManager.xml'
...
def Plugins(**kwargs):
    iconpic = 'logo.png'
    if isDreamOS:
        iconpic = resolveFilename(SCOPE_PLUGINS, "Extensions/nssManager/res/pics/tvlogofhd.png")
