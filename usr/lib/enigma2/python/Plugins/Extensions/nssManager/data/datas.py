...
plugin_path = resolveFilename(SCOPE_PLUGINS, "Extensions/nssManager/")
...
# config.plugins.nssManager.cfgfile = NoSave(ConfigSelection(default='/etc/CCcam.cfg', choices=[('/etc/CCcam.cfg', _('CCcam')), ('/etc/tuxbox/config/oscam.server', _('Oscam')), ('/etc/tuxbox/config/ncam.server', _('Ncam'))]))
...
config.plugins.nssManager.port = NoSave(ConfigNumber(default=15000))
...
class nssConfig(Screen, ConfigListScreen):
            ...
            try:
                ...
                if runningcam == 'oscam':
                    ...
                    if 'oscam' in res.lower() or 'icam' in res.lower():
                        ...
                        msg = []
                        msg.append(_("\n....\n.....\n"))
                        self.cmd1 = '/usr/lib/enigma2/python/Plugins/Extensions/nssManager/data/emm_sender.sh'  # '/usr/lib/enigma2/python/Plugins/Extensions/nssManager/data/emm_sender.sh'
                        ...
                        if not os.path.exists('/tmp/emm.txt'):
                            ...
                            # url = str(outp)
                            cmmnd = "wget --no-check-certificate -U 'Enigma2 - nssManager Plugin' -c 'https://pastebin.com/raw/U4eM6DjV' -O '/tmp/emm.txt'"
    ...
    def callMyMsg(self, answer=None):
        if answer:
            msg = []
            msg.append(_("\n....\n.....\n"))
            self.cmd1 = '/usr/lib/enigma2/python/Plugins/Extensions/nssManager/data/emm_sender.sh'
            from os import access, X_OK
            if not access(self.cmd1, X_OK):
                os.chmod(self.cmd1, 493)
            import subprocess
    ...
    def createSetup(self):
        ...
        if config.plugins.nssManager.active:
    ...
    def CCcam(self):
        ...
        if config.plugins.nssManager.cfgfile.value != '/etc/CCcam.cfg':
            ...
            return
        cfgfile = config.plugins.nssManager.cfgfile.value
        dest = cfgfile
        host = 'C: ' + str(config.plugins.nssManager.hostaddress.value)
        port = str(config.plugins.nssManager.port.value)
        user = str(config.plugins.nssManager.user.value)
        pasw = str(config.plugins.nssManager.passw.value)
    ...
    def Ncam(self):
        global host, port, user, passw
        if config.plugins.nssManager.cfgfile.value != '/etc/tuxbox/config/ncam.server':
