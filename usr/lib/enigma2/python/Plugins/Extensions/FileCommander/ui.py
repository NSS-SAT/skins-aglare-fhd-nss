...
class FileCommanderScreen(Screen, HelpableScreen, key_actions):
	...
	def __init__(self, session, path_left=None):
		...
		self["actions"] = HelpableActionMap(self, ["FileCommanderActions"], {
			"FCok": (self.ok, _("Play/view/edit/install/extract/run file or enter directory")),
			"FCcancel": (self.exit, _("Leave File Commander")),
			"FCmenu": (self.selectAction, _("Open settings/actions menu")),
			"FCnext": (self.listRight, _("Activate right-hand file list as source")),
			"FCback": (self.listLeft, _("Activate left-hand file list as source")),
			"FCpageup": (self.listRightB, _("Activate right-hand file list as source")),
			"FCpagedown": (self.listLeftB, _("Activate left-hand file list as source")),
			"FC1": (self.gomakeDir, _("Create directory")),
			"FC2": (self.gomakeSym, _("Create user-named symbolic link")),
			"FC3": (self.gofileStatInfo, _("File/Directory Status Information")),
			"FC4": (self.call_change_mode, _("Change execute permissions (755/644)")),
			"FC5": (self.goDefaultfolder, _("Go to bookmarked folder")),
			"FC6": (self.run_file, self.help_run_file),
			"FC7": (self.run_ffprobe, self.help_run_ffprobe),
			"FC8": (self.dirSizeWalk, _("Counting directory content size while walking (on/off)")),
			"FC9": (self.run_hashes, _("Calculate file checksums")),
			"FCtext": (self.file_viewer, _("View or edit file (if size < 1MB)")),
			"FCinfo": (self.openTasklist, _("Show task list")),
			"FCepg": (self.goParentfolder, _("Go to parent directory")),
			"FCup": (self.goUp, _("Move up list")),
			"FCdown": (self.goDown, _("Move down list")),
			"FCleft": (self.goLeftB, _("Page up list")),
			"FCright": (self.goRightB, _("Page down list")),
			"FCred": (self.goRed, _("Delete file or directory (and all its contents)")),
			"FCgreen": (self.goGreen, _("Move file/directory to target directory")),
			"FCyellow": (self.goYellow, _("Copy file/directory to target directory")),
			"FCblue": (self.goBlue, _("Rename file/directory")),
			"FC0": (self.doRefresh, _("Refresh screen")),
			"FCvideo": (self.listSelect, _("Enter multi-file selection mode")),
			"FCpvr": (self.listSelect, _("Enter multi-file selection mode")),
			"FCredlong": (self.goRedLong, _("Sorting left files by name, date or size")),
			"FCseekBack": (self.goRedLong, _("Sorting left files by name, date or size")),
			"FCgreenlong": (self.goGreenLong, _("Reverse left file sorting")),
			"FCyellowlong": (self.goYellowLong, _("Reverse right file sorting")),
			"FCbluelong": (self.goBlueLong, _("Sorting right files by name, date or size")),
			"FCseekFwd": (self.goBlueLong, _("Sorting right files by name, date or size")),
			"FChelpshow": (self.showHelpMenu, _("FileCommander Help")),
		}, -1)
	...
	def selectAction(self):
		...
		menu.append((_("Show task list"), self.openTasklist))																# info
		menu.append((_("Refresh screen"), self.doRefresh))																	# 0
		menu.append((_("Create directory"), self.gomakeDir))																# 1
		menu.append((_("Create user-named symbolic link"), self.gomakeSym))													# 2
		menu.append((_("File/Directory Status Information"), self.gofileStatInfo))											# 3
		keys = ["menu", "info", "0", "1", "2", "3"]
		if isFile:
			menu.append((_("Change execute permissions (755/644)"), self.call_change_mode))									# 4
			keys += ["4"]
		menu.append((_("Go to default directory"), self.goDefaultfolder))													# 5
		menu.append((self.help_run_file(), self.run_file))																	# 6
		menu.append((self.help_run_ffprobe(), self.run_ffprobe))															# 7
		menu.append((_("Counting directory content size while walking (on/off)"), self.dirSizeWalk))						# 8
		keys += ["5", "6", "7", "8"]
		if isFile:
			menu.append((_("Calculate file checksums"), self.run_hashes))													# 9
			keys += ["9"]
		menu.append((_("Delete file or directory (and all its contents)"), self.goRed))										# red
		menu.append((_("Move file/directory to target directory"), self.goGreen))											# green
		menu.append((_("Copy file/directory to target directory"), self.goYellow))											# yellow
		menu.append((_("Rename file/directory"), self.goBlue))																# blue
		keys += ["red", "green", "yellow", "blue"]
		if isFile:
			menu.append((_("View or edit file (if size < 1MB)"), self.file_viewer))											# text
			keys += ["text"]
		menu.append((_("FileCommander Help"), self.showHelpMenu))															# help
		menu.append((_("Enter multi-file selection mode"), self.listSelect))												#
		keys += ["help", "bullet"]
...
class FileCommanderScreenFileSelect(Screen, HelpableScreen, key_actions):
	def __init__(self, session, parent, leftactive, selectedid):
		...
		self.skinName = "FileCommanderScreen"
		...
		self["actions"] = HelpableActionMap(self, ["FileCommanderActions"], {
			"FCmenu": (self.selectAction, _("Open actions menu")),
			"FCok": (self.ok, _("Select (source list) or enter directory (target list)")),
			"FCcancel": (self.exit, _("Leave multi-select mode")),
			"FCnext": (self.listRight, _("Activate right-hand file list as multi-select source")),
			"FCback": (self.listLeft, _("Activate left-hand file list as multi-select source")),
			"FCpageup": (self.listRightB, _("Activate right-hand file list as multi-select source")),
			"FCpagedown": (self.listLeftB, _("Activate left-hand file list as multi-select source")),
			"FCup": (self.goUp, _("Move up list")),
			"FCdown": (self.goDown, _("Move down list")),
			"FCleft": (self.goLeftB, _("Page up list")),
			"FCright": (self.goRightB, _("Page down list")),
			"FCred": (self.goRed, _("Delete the selected files or directories")),
			"FCgreen": (self.goGreen, _("Move files/directories to target directory")),
			"FCyellow": (self.goYellow, _("Copy files/directories to target directory")),
			"FCblue": (self.invertSelection, _("Invert selection")),
			"FCinfo": (self.gofileStatInfo, _("File/Directory Status Information")),
			"FC0": (self.doRefresh, _("Refresh screen")),
			"FC1": (self.dirSizeWalk, _("Counting directory content size while walking (on/off)")),
			"FC2": (boundFunction(self.selectGroup, True), _("Select group")),
			"FC5": (boundFunction(self.selectGroup, False), _("Deselect group")),
			"FC6": (self.moveSelector, _("Move selector to next item (on/off)")),
			"FC8": (self.openTasklist, _("Show task list")),
			"FCvideo": (self.goBlue, _("Leave multi-select mode")),
			"FCpvr": (self.goBlue, _("Leave multi-select mode")),
			"FChelpshow": (self.showHelpMenu, _("FileCommander Help")),
		}, -1)
		self.selItems = 0
		self.selSize = 0
...
#			print "[FileCommander] selectedFiles:", self.selectedFiles
	...
	def selectAction(self):
		...
		menu.append((_("FileCommander Help"), self.showHelpMenu))
		keys = ["2", "5", "", "", "7", "blue", "menu", "help"]
		self.session.openWithCallback(self.menuCallback, ChoiceBox, title=_("Select operation:"), list=menu, keys=["dummy" if key == "" else key for key in keys], skin_name="ChoiceBox")
