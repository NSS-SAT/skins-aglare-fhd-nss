...
def ensure_str(s, encoding='utf-8', errors='strict'):
    """Coerce *s* to `str`.
    Works for both Python 2 and Python 3 using the `six` library.
    """
    # Optimization: Fast return for the common case.
    if type(s) is str:
        return s
    if isinstance(s, text_type):  # Unicode in Python 2, str in Python 3
        return s.encode(encoding, errors) if PY2 else s
    elif isinstance(s, binary_type):  # str in Python 2, bytes in Python 3
...
def show_(name, link):
    ...
    base_skin_path = os_path.dirname(resolveFilename(SCOPE_SKIN, str(cur_skin)))
    default_icon = os_path.join(base_skin_path, "vavoo", "vavoo_ico.png")
    country_code = country_codes.get(name, None)
    if country_code:
        country_code = country_code + '.png'
        pngx = os_path.dirname(resolveFilename(SCOPE_SKIN, country_code))
    else:
        pngx = default_icon
...
class AutoStartTimer:
    ...
    def startMain(self):
        name = url = ''
        favorite_channel = os_path.join(enigma_path, 'Favorite.txt')
